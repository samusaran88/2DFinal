...
void image::enlargeFrameRender(HDC hdc, int destX, int destY, int currentFrameX, int currentFrameY, float multiRate)
{
	GdiTransparentBlt(hdc, destX, destY, 
		_imageInfo->frameWidth * multiRate, _imageInfo->frameHeight * multiRate, _imageInfo->hMemDC,
...
HRESULT image::initTrans(int width, int height, BOOL trans, COLORREF transColor, BOOL alphablend)
{
	//이미지 정보가 뭔가있다면 해제해줘라
	if (_imageInfo != NULL) release();
